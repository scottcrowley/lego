...
<div class="w-full px-3 sm:px-0">
    <div class="w-2/3 mb-3 mb-8 mx-auto border rounded-lg p-6 bg-white shadow-md">
        <div>
            ...
            <p class="mb-2 font-hairline tracking-wide"><span class="font-semibold">Parts in Location:</span> {{ $location->parts->unique('part_num')->count() }}</p>
            <p class="mb-2 font-hairline tracking-wide"><span class="font-semibold">Individual Parts in Location:</span> {{ $location->parts->count() }}</p>
            <p class="mb-2 font-hairline tracking-wide"><span class="font-semibold">Total Pieces in Location:</span> {{ $location->parts->sum('quantity') }}</p>
        </div>
        <div class="pt-2">
            <a href="{{ route('storage.locations.parts.edit', $location->id) }}" class="btn is-primary is-narrow">Edit Parts in Location</a>
            <a href="{{ route('storage.locations.parts.index', $location->id) }}" class="btn is-primary is-narrow">View Grouped Parts</a>
        </div>
    </div>
